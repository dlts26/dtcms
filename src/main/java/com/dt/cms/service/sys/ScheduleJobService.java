...
public class ScheduleJobService {
	...
	@Autowired
	private Scheduler quatzScheduler;
	...
	public void add(ScheduleJob scheduleJob) {
		...
		try {
			quatzScheduler.scheduleJob(jobDetail, trigger);
			log.info("定时任务添加成功");
	...
	public List<JobDetail> getJobs() {
		try {
			...
			Set<JobKey> jobKeys = quatzScheduler.getJobKeys(matcher);
			List<JobDetail> jobDetails = new ArrayList<JobDetail>();
			for (JobKey key : jobKeys) {
				jobDetails.add(quatzScheduler.getJobDetail(key));
			}
			return jobDetails;
		} catch (SchedulerException e) {
			e.printStackTrace();
	...
	public List<ScheduleJob> getAllScheduleJob() {
		...
		try {
			...
			for (JobKey jobKey : jobKeys) {
				List<? extends Trigger> triggers = quatzScheduler.getTriggersOfJob(jobKey);
				for (Trigger trigger : triggers) {
					ScheduleJob scheduleJob = new ScheduleJob();
					scheduleJob.setName(jobKey.getName());
					scheduleJob.setGroup(jobKey.getGroup());
					Trigger.TriggerState triggerState = quatzScheduler.getTriggerState(trigger.getKey());
					scheduleJob.setStatus(triggerState.name());
					// 获取要执行的定时任务类名
					JobDetail jobDetail = quatzScheduler.getJobDetail(jobKey);
	...
	public List<ScheduleJob> getAllRuningScheduleJob() {
		...
		try {
			List<JobExecutionContext> executingJobs = quatzScheduler.getCurrentlyExecutingJobs();
	...
	public List<ScheduleJob> getTriggersInfo() {
		try {
			...
			Set<TriggerKey> Keys = quatzScheduler.getTriggerKeys(matcher);
			List<ScheduleJob> triggers = new ArrayList<ScheduleJob>();

			for (TriggerKey key : Keys) {
				Trigger trigger = quatzScheduler.getTrigger(key);
				ScheduleJob scheduleJob = new ScheduleJob();
				scheduleJob.setName(trigger.getJobKey().getName());
				scheduleJob.setGroup(trigger.getJobKey().getGroup());
				scheduleJob.setStatus(quatzScheduler.getTriggerState(key) + "");
				if (trigger instanceof SimpleTrigger) {
					SimpleTrigger simple = (SimpleTrigger) trigger;
					scheduleJob.setCronExpression(
							"重复次数:" + (simple.getRepeatCount() == -1 ? "无限" : simple.getRepeatCount()) + ",重复间隔:"
	...
	public void startNowJob(String name, String group) {
		...
		try {
			quatzScheduler.triggerJob(jobKey);
		} catch (SchedulerException e) {
			e.printStackTrace();
		}
	}
	...
	public void modifyTrigger(String name, String group, String cron) {
		try {
			...
			quatzScheduler.rescheduleJob(key, newTrigger);
		} catch (SchedulerException e) {
			e.printStackTrace();
		}
